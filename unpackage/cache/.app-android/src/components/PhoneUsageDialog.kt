...
open class GenComponentsPhoneUsageDialog : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenComponentsPhoneUsageDialog) -> Any? = fun(__props): Any? {
            ...
            fun gen_arcPath_fn(startPct: Number, endPct: Number): String {
                if (endPct <= startPct) {
                    return ""
                }
                ...
                val startRad = (startPct / 100) * 2 * Math.PI - Math.PI / 2
                val endRad = (endPct / 100) * 2 * Math.PI - Math.PI / 2
                val cx: Number = 50
                val cy: Number = 50
                val r: Number = 40
                ...
                val largeArc = if ((endPct - startPct) > 50) {
                    1
                } else {
                    0
                }
            ...
            val arcPath = ::gen_arcPath_fn
            fun gen_getSvgSegments_fn(): UTSArray<SvgSegment> {
                val r: UTSArray<SvgSegment> = _uA()
                run {
                    var i: Number = 0
                    while(i < segments.value.length){
                        val s = segments.value[i]
                        val d = arcPath(s.startAngle, s.endAngle)
                        r.push(SvgSegment(d = d, color = s.color))
                        i++
                    }
                }
                return r
            }
            val getSvgSegments = ::gen_getSvgSegments_fn
            val svgSegments = computed<UTSArray<SvgSegment>>(getSvgSegments)
            ...
            return fun(): Any? {
                val _component_circle = resolveComponent("circle")
                val _component_path = resolveComponent("path")
                val _component_svg = resolveComponent("svg")
                        ...
                        _cE("view", _uM("class" to "dialog-card", "onClick" to withModifiers(fun(){}, _uA(
                            ...
                            if (unref(segments).length > 0) {
                                _cE("view", _uM("key" to 0, "class" to "chart-block"), _uA(
                                    _cE("view", _uM("class" to "pie-svg-wrap"), _uA(
                                        _cV(_component_svg, _uM("viewBox" to "0 0 100 100", "class" to "pie-svg"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                            return _uA(
                                                _cV(_component_circle, _uM("cx" to "50", "cy" to "50", "r" to "40", "fill" to "#F0F3F4")),
                                                _cE(Fragment, null, RenderHelpers.renderList(unref(svgSegments), fun(seg, si, __index, _cached): Any {
                                                    return _cV(_component_path, _uM("key" to ("sg-" + si), "d" to seg.d, "fill" to seg.color, "stroke" to "#FFFFFF", "stroke-width" to "0.5"), null, 8, _uA(
                                                        "d",
                                                        "fill"
                                                    ))
                                                }), 128),
                                                _cV(_component_circle, _uM("cx" to "50", "cy" to "50", "r" to "22", "fill" to "#FFFFFF")),
                                                _cE("text", _uM("x" to "50", "y" to "48", "text-anchor" to "middle", "font-size" to "6", "fill" to "#7F8C8D"), "总占比"),
                                                _cE("text", _uM("x" to "50", "y" to "56", "text-anchor" to "middle", "font-size" to "7", "fill" to "#2C3E50", "font-weight" to "bold"), _tD(unref(summary).appCount) + "个", 1)
                                            )
                                        }), "_" to 1))
                                    )),
